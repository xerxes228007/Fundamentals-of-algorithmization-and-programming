...
void Visual::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
QVector<int> Visual::generateRandomArray(int n)
{
    QVector<int> result;
    result.reserve(n);
...
QVector<int> Visual::getColor(int n)
{
    QVector<int> result;
    result.reserve(n);
...
template <typename T>
void Visual::swap(T &a, T &b, int one, int two)
{
    color[one] = 1;
    color[two] = 1;
    update();
...
template <typename T>
void Visual::heapify(QVector<T> &arr, int n, int i)
{
    int largest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;
...
template <typename T>
void Visual::heapSort(QVector<T> &arr)
{
    int n = arr.size();
...
template <typename T>
int Visual::partition(QVector<T> &arr, int low, int high)
{
    int pivot = arr[high];
    int i = (low - 1);
...
template <typename T>
void Visual::quickSort(QVector<T> &arr, int low, int high)
{
    if (low < high)
    {
...
template <typename T>
void Visual::merge(QVector<T> &arr, int left, int mid, int right)
{
    int n1 = mid - left + 1;
    int n2 = right - mid;
...
template <typename T>
void Visual::mergeSort(QVector<T> &arr, int left, int right)
{
    if (left < right)
    {
...
void Visual::interpolationSorting()
{
    allowUpdateNumber = false;
    if (allowSort)
    {
...
template <typename T>
void Visual::interpolationSort(QVector<T>& arr, int low, int high, int minVal, int maxVal) {
    if (low < high) {
...
int Visual::binarySearch(int target)
{
    for (int i = 0; i < number; i++)
    {
        color[i] = 0;
