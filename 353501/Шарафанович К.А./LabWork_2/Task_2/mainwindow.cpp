...
void MainWindow::on_pushButton_save_clicked()
{
    ...
    for (auto i : details) {
        out << i.GetGroup() << ' ' << i.GetName() << ' ' << i.GetPriznak() << ' ' << i.GetRemark() << ' ' << i.GetAmount() << ' ' << i.isAvaliable() << '\n';
    }
    file.close();
}
...
void MainWindow::arrayToTable(QVector<Detail> arr){
    ...
    for (auto i : arr) {
        ...
        pCell->setText(QString::number(i.GetAmount()));
...
void MainWindow::on_pushButton_search_clicked()
{
   ...
   if (ui->radioButton_company->isChecked()) {
        ...
        for (auto i : details) {
            if (i.GetPriznak() == compare)
                fi.push_back(i);
        }
        }else if (ui->radioButtona_group->isChecked()){
               for (auto i : details) {
                   if (i.GetGroup() == compare)
                       fi.push_back(i);
               }
        }
...
}void MainWindow::on_pushButton_sort_clicked()
{
    ...
    for (int i = 0; i < sortStrings.size() - 1; ++i) {
        for (int j = 0; j < sortStrings.size() - 1; ++j) {
            if (sortStrings[j].GetGroup() < sortStrings[j + 1].GetGroup()){
    ...
    for (int i = 1; i < sortStrings.size(); ++i) {
        if (sortStrings[i - 1].GetGroup() != sortStrings[i].GetGroup()){
            aa.push_back(temp);
            temp.clear();
        }
    ...
    for (auto& k : aa) {
        for (int i = 0; i < k.size() - 1; ++i) {
            for (int j = 0; j < k.size() - 1; ++j) {
                if (k[j].GetPriznak() == '1') {  // Если GetPriznak равен 1
                    if (k[j].GetGroup() > k[j + 1].GetGroup()) {
                        std::swap(k[j], k[j + 1]);
                    }
                } else {
                    if (k[j].GetPriznak() > k[j + 1].GetPriznak()) {
                        std::swap(k[j], k[j + 1]);
                    }
                }
            }
