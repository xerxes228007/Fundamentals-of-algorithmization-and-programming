...
public:
    ...
    void remove(int key, node *v, node *&pred, bool left) {
        ...
        if (key == v->key) { // Found
            if (v->left == v->right && v->left == nullptr) { // Leaf
                delete v;
                if (pred == nullptr) { // Root node
                    root = nullptr;
                    return;
                }
                if (left)
                    ...
                    pred->right = nullptr;
            } else if (v->left == nullptr) { // No left child
                if (pred == nullptr) { // Root node
                    root = v->right;
                } else {
                    if (left)
                ...
                delete v;
            } else if (v->right == nullptr) { // No right child
                if (pred == nullptr) { // Root node
                    root = v->left;
                } else {
                    if (left)
                ...
                node *Successor = v->right; // Find minimum in right subtree
