...
void MainWindow::moveUp(int index)
{
    ...
    *upd = true;
    ...
    connect(tm, &QTimer::timeout, [=](){
        ...
        if (circleFigure[circleFigure.size() - vec[index][0]]->y() < 670 - numbOfColonmn * 30 || abs(circleFigure[circleFigure.size() - vec[index][0]]->x() - endX) > 10){
            ...
            tm->stop();
            circleFigure[circleFigure.size() - vec[index][0]]->setRect(endX, 670 - numbOfColonmn * 30, circleFigure[circleFigure.size() - vec[index][0]]->width(), 30);
            *upd = false;
        }
    });
    tm->start(30);
...
void MainWindow::renul()
{
    ...
    table->setHorizontalHeaderLabels(QStringList{"Number", "From", "To"});
    table->setGeometry(1000, 0, 325, 1000);
