...
public:
    ...
    TreeNode(const QString &data, int key)
        : data(data), key(key), height(1), left(nullptr), right(nullptr) {}
};
...
private:
    ...
    void insertSubtree(TreeNode *&destNode, TreeNode *subtree)
    {
        if (!destNode)
        {
            destNode = subtree;
        ...
        {
            if (subtree->key < destNode->key)
            {
                insertSubtree(destNode->left, subtree);
            }
            else
            {
                insertSubtree(destNode->right, subtree);
            }
        }
        destNode = balanceNode(destNode);
    }

    void removeSubtree(TreeNode *&node, int key)
    {
        if (!node)
            return;
        if (key < node->key)
        {
            removeSubtree(node->left, key);
        }
        else if (key > node->key)
        {
            removeSubtree(node->right, key);
        }
        else
        {
            clear(node);
    ...
    void insertBranch(TreeNode *&destNode, TreeNode *branch)
    {
        if (!destNode)
        {
            destNode = branch;
        ...
        {
            if (branch->key < destNode->key)
            {
                insertBranch(destNode->left, branch);
            }
            else
            {
                insertBranch(destNode->right, branch);
            }
        }
        destNode = balanceNode(destNode);
    }

    void removeBranch(TreeNode *&node, int key)
    {
        if (!node)
            return;
        if (key < node->key)
        {
            removeBranch(node->left, key);
        }
        else if (key > node->key)
        {
            removeBranch(node->right, key);
        }
        else
        {
            if (node->left)
    ...
    TreeNode *findLowestCommonAncestor(TreeNode *node, int key1, int key2)
    {
        if (!node)
            return nullptr;
...
public:
    ...
    {
        return treeStruct(root);
    }

    void insertSubtree(const QString &data, int key, TreeNode *subtree)
    {
    ...
    void insertBranch(const QString &data, int key, TreeNode *branch)
    {
        TreeNode *newNode = new TreeNode(data, key);
        insertBranch(root, newNode);
    }

    void removeBranch(int key)
    {
        removeBranch(root, key);
    }

    TreeNode *findLowestCommonAncestor(int key1, int key2)
    {
