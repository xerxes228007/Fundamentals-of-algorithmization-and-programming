...
int mangle_key(long key) { 
  return (key * 893923 + 1152931) % 10000007; 
}
...
TEST(combine, simple) {
  ...
  SearchTree<int> tree2;
  for(int i = 0; i < 1000; i+=2) {
    tree1.insert(mangle_key(i), 1000 + i);
    tree2.insert(mangle_key(2000 + i + 1), 2000 + i);
  }


  SearchTree<int> tree = tree1;
  tree.appendTree(tree2);
  for(int i = 0; i < 1000; i++) {
    if(i % 2 == 0) {
      EXPECT_EQ(tree.get(mangle_key(i)), 1000 + i);
    } else {
      EXPECT_EQ(tree.get(mangle_key(2000+i)), 2000 + i - 1);
    }
  }
}


TEST(combine, commonRoot) {
  SearchTree<int> tree;
  ...
  EXPECT_EQ(tree.findCommonKey(4, 6), 5);
}