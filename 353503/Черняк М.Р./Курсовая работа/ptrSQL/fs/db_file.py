...
class DBFile(BlockStorage):
    ...
    def master_table(self) -> RowSet:
        gato = Identifier('gato', True)
        columns = [
            Column(gato, Identifier('type'), VarCharTy(16)),
            Column(gato, Identifier('name'), VarCharTy(255)),
            Column(gato, Identifier('sql'), VarCharTy(1024)),
        ]
        rows: List[List[str]] = []
        for table in self.get_tables():
