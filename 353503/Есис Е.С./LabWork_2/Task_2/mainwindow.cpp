...
{
    ui->setupUi(this);
    ui->topScorersTable->hide();//скрывает
    ui->table->show();//показывает
    ...
    ui->frame->show();
    ui->EDITMENU->hide();
    ui->table->setEditTriggers(QAbstractItemView::NoEditTriggers);//только просмотр таблицы
...
void MainWindow::FromFiletoVector(){
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text))//проверка на открытие файла
    {
    ...
    while (!in.atEnd())//выполняется до тех пор, пока не достигнут конец файла
    {
