...
private:
    ...
    struct Node {
        ...
        Node() = default;
        Node(Key key, Value value) : data(key, value) {}
    };
    ...
    std::shared_ptr<Node> findNode(const std::shared_ptr<Node>& curNode, const Key& key) const {
        ...
        } else if (key > curNode->data.first) {
    ...
    void inorderTraversal(std::shared_ptr<Node> node, QString& str, int lvl) {
        if (node) {
            ...
            if (node == _nil) {
                ...
                str += "\n" + tab + (node->bIsRed ? " Red " : " Black ") +
                       QString::number(node->data.first) + "K " + QString::number(node->data.second) + "V";
            }
