...
Sprite::Sprite(std::shared_ptr<Texture2D> pTexture,
    ...
    : m_pTexture(std::move(pTexture)),
      m_pShaderProgram(std::move(pShaderProgram)), m_lastFrameId(0) {
  const GLfloat vertexCoords[] = {0.f, 0.f, 0.f,1.f,
                                  1.f, 1.f, 1.f, 0.f};
...
void Sprite::render(const glm::vec2 &position, const glm::vec2 &size,
  ...
  if (m_lastFrameId != frameId) {
    ...
    const FrameDescription &currentFrameDescription =
        m_framesDescriptions[frameId];

    const GLfloat textureCoords[] = {
        // U  V
        currentFrameDescription.leftBottomUV.x,
        currentFrameDescription.leftBottomUV.y,
        currentFrameDescription.leftBottomUV.x,
        currentFrameDescription.rightTopUV.y,
        currentFrameDescription.rightTopUV.x,
        currentFrameDescription.rightTopUV.y,
        currentFrameDescription.rightTopUV.x,
        currentFrameDescription.leftBottomUV.y,
    };
...
size_t Sprite::getFramesCount() const { return m_framesDescriptions.size(); }
} // namespace RenderEngine