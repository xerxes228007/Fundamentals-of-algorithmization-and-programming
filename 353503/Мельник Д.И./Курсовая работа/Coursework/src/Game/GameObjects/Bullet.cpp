...
Bullet::Bullet(const double velocity, const glm::vec2 &position,
               const glm::vec2 &size, const glm::vec2 &explosionSize,
               const float layer)
        : IGameObject(IGameObject::EObjectType::Bullet, position, size, 0.f, layer),
          ...
          m_eOrientation(EOrientation::Top), m_maxVelocity(velocity),
          m_isExplosion(false){
    ...
    if (m_isExplosion) {
        switch (m_eOrientation) {
            case EOrientation::Top:
                m_pSprite_explosion->render(m_position - m_explosionOffset +
                                            glm::vec2(0, m_size.y / 2.f),
                                            m_explosionSize, m_rotation, m_layer + 0.1f,
                                            m_spriteAnimator_explosion.getCurrentFrame());
                break;
...
void Bullet::fire(const glm::vec2 &position, const glm::vec2 &direction) {
    ...
    if (m_direction.x == 0.f) {
        m_eOrientation =
                (m_direction.y < 0) ? EOrientation::Bottom : EOrientation::Top;
    } else {
        m_eOrientation =
                (m_direction.x < 0) ? EOrientation::Left : EOrientation::Right;
    }
    m_isActive = true;
