...
ResourceManager::loadShaders(const std::string &shaderName, const std::string &vertexPath,
    ...
    std::shared_ptr<RenderEngine::ShaderProgram> &newShader
            = m_shaderPrograms.emplace(shaderName, std::make_shared<RenderEngine::ShaderProgram>(vertexString,
...
std::shared_ptr<RenderEngine::Sprite> ResourceManager::loadSprite(const std::string &spriteName,
    ...
    std::shared_ptr<RenderEngine::Sprite> newSprite = m_sprites.emplace(
            ...
            std::make_shared<RenderEngine::Sprite>(
                    pTexture, subTextureName, pShader)).first->second;
    ...
    if (pTexture) {
        ...
        for (auto &currentSubTextureName: subTextures) {
            glm::vec2 leftBottomUV(static_cast<float>(currentTextureOffsetX + 0.01f) / textureWidth,
                                   static_cast<float>(currentTextureOffsetY - subTextureHeight + 0.01f) /
                                   textureHeight);
