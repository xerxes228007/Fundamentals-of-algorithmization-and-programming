...
namespace Physics {
    ...
    void PhysicsEngine::update(const double delta) {
        ...
        for (auto it1 = m_dynamicObjects.begin(); it1 != m_dynamicObjects.end();) {
            ...
            for (auto it2 = ++it1; it2 != m_dynamicObjects.end(); ++it2) {
                auto &pObject2 = *it2;
                if (pObject2->isActive() and pObject1->isActive()) {
    ...
    void PhysicsEngine::calculateTargetPositions(std::unordered_set<std::shared_ptr<IGameObject>> &dynamicObjects,
                                                 const double delta) {
        for (auto &currentDynamicObject: dynamicObjects) {
            if (currentDynamicObject->getCurrentVelocity() > 0) {
                ...
                if (currentDynamicObject->getCurrentDirection().x !=
                    0.f) // right and left
                {
                ...
                ECollisionDirection dynamicObjectCollisionDirection =
                        ECollisionDirection::Right;
    ...
    void PhysicsEngine::updatePositions(
            std::unordered_set<std::shared_ptr<IGameObject>> &dynamicObjects) {
        for (auto &currentDynamicObject: dynamicObjects) {
    ...
    bool PhysicsEngine::hasPositionIntersection(const std::shared_ptr<IGameObject> &pObject1,
                                                const glm::vec2 &position1,
                                                const std::shared_ptr<IGameObject> &pObject2,
        ...
        for (const auto &currentObjectCollider: currentObjectColliders) {
            for (const auto &otherObjectCollider: otherObjectColliders) {
                if (hasCollidersIntersection(currentObjectCollider, position1,
                                             otherObjectCollider, position2)) {
                    return true;
                }
            }
        }
    ...
    bool PhysicsEngine::hasCollidersIntersection(const Collider &collider1,
                                                 const glm::vec2 &position1,
