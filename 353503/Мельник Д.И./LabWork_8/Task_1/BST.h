#ifndef B6BB4A3E_50E9_49CF_B08D_C89AEA356769
#define B6BB4A3E_50E9_49CF_B08D_C89AEA356769
#include <cstddef>
#include <cstdlib>
#include <stdexcept>
#include <sys/types.h>
...
public:
  ...
  node *_lca(node *rt, node *n1, node *n2) {
    auto v1 = n1->val;
    auto v2 = n1->val;

    if (v1 > rt->val && v2 > rt->val) {
      return _lca(rt->right, n1, n2);
    } else if (v1 < rt->val && v2 < rt->val) {
      return _lca(rt->left, n1, n2);
    } else {
      return rt;
    }
  }
  ...
  T lca(T t1, T t2) {
    ...
    if (!n1.is_valid() || !n2.is_valid()) {
      throw std::runtime_error("No elements in a tree\n");
    }
    return _lca(n1.nd, n2.nd);
  }


  void merge(const BST<T>& other){
    auto vect1 = this->in_order_traverse();
    auto vect2 = other.in_order_traverse();
    vect1.insert(vect1.end(), vect2.begin(), vect2.end());
    std::sort(vect1.begin(), vect1.end());
    release_tree(root);
    root = build_balanced_from_sorted(vect1, 0, vect1.size() - 1);
  }
...
#endif /* B6BB4A3E_50E9_49CF_B08D_C89AEA356769 */
