...
void MainWindow::on_comboBox_currentIndexChanged(int index)
{
    ...
    if (index == bel) {
        currText
            = "На небе свеціць яркае сонца, а птушкі спяваюць у садзе.Вясной усё навокал ажывае, і людзі выходзяць на прогулку.";
    } else if (index == arab) {
        currText = "مدرب لوحة المفاتيح الغبي";
    } else if (index == ger) {
        currText = "Der schnelle braune Fuchs springt über den faulen Hund.Heute ist ein wunderschöner Tag, um im Park spazieren zu gehen.";
    } else if (index == fre) {
        currText = "Le renard brun rapide saute par-dessus le chien paresseux.Aujourd’hui, le ciel est clair et c’est le moment idéal pour une balade en vélo.";
    } else if (index == chin) {
        currText = "zhong1 guo2 cheng2 wei2 ya4 zhou1 zui4 jia1 lv3 you2 mu4 de di4 ";
    } else if (index == heb) {
        currText = "יהודים הם בחורים קשוחים, במיוחד בבדיחות";
    } else if (index == ru) {
        currText = "Быстрый коричневый лис прыгает через ленивую собаку.Сегодня отличный день, чтобы отправиться на прогулку по берегу реки.";
    }
    mask.fill('c', currText.size());
    printText();
}
