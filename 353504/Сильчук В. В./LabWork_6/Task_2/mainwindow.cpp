...
void MainWindow::printMenu(int choice)
{
    ...
    case 11:
        ...
        break;
    default:
        outputStream << "Ошибка. Выберите пункт меню от 1 до 11.\n";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
    }
    outputStream << "\n";
...
{
    QStringList filesToCreate = {"SilchukLab6.txt", "SilchukLab6_0.txt", "SilchukLab6_1.txt", "SilchukLab6_2.bin", "SilchukLab6_3.txt"};
...
void MainWindow::saveCharArrayToFile(const QString &fileName)
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        outputStream << "Введите размер массива";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
...
void MainWindow::saveAnimalDataToFile(const QString &fileName)
{
    ...
    if (!file.open(QIODevice::Append | QIODevice::Text)) {
        ...
        ui->lineEdit->setText(textInput);
        ui->lineEdit->clear();
        outputStream << "Введите вес(double)";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
        ...
        ui->lineEdit->setText(textInput);
        ui->lineEdit->clear();
        outputStream << "Введите тип диеты (один символ)";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
        ...
        ui->lineEdit->setText(textInput);
        ui->lineEdit->clear();
        outputStream << "Введите является ли мужским (0 или 1)";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
