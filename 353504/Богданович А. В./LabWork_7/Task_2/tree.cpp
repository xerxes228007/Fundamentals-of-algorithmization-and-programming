...
template<typename T, typename U>
void Tree<T, U>::symmetricalPrint(QLabel *label)
{
    symmetricalPrint(root, label);
}

template<typename T, typename U>
void Tree<T, U>::reversePrint(QLabel *label)
{
    reversePrint(root, label);
}

template<typename T, typename U>
void Tree<T, U>::straightPrint(QLabel *label)
{
    straightPrint(root, label);
}

template<typename T, typename U>
void Tree<T, U>::symmetricalPrint(Node<T, U> *node, QLabel* label) //возрастание
{
    if(node == nullptr) return;
    symmetricalPrint(node->left, label);
    label->setText(label->text() + (node->value) + " | ");
    symmetricalPrint(node->right, label);
}

template<typename T, typename U>
void Tree<T, U>::reversePrint(Node<T, U> *node, QLabel* label) //delete
{
    if(node == nullptr) return;
    reversePrint(node->left, label);
    reversePrint(node->right, label);
    label->setText(label->text() + (node->value) + " | ");
}

template<typename T, typename U>
void Tree<T, U>::straightPrint(Node<T, U> *node, QLabel* label) //copy
{
    if(node == nullptr) return;
    label->setText(label->text() + (node->value) + " | ");
    straightPrint(node->left, label);
    straightPrint(node->right, label);
}
