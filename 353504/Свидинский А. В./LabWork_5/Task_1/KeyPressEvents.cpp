...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    if (code == 66) {
        currCaps = !currCaps;
        QString backgroundColor = currCaps ? randomColor() : "#f0f0f0";
        QString textColor = "#000000";
        QString styleSheet = QString("QPushButton:disabled {border: 5px solid #000065; background-color: %1; color: %2;}").arg(backgroundColor, textColor);
    ...
    // Обработка нажатия Shift
        ...
        QString backgroundColor = randomColor();
        QString textColor = "#000000";
        QString styleSheet = QString("QPushButton:disabled {border: 5px solid #000065; background-color: %1; color: %2;}").arg(backgroundColor, textColor);
...
void MainWindow::keyReleaseEvent(QKeyEvent *event)
{
    ...
    if (code < 67 && currButtons[code - 1] != nullptr && currButtons[code - 1]) {
        if (code != 66) {
            if (code == 50 || code == 62) {
                QString styleSheet = "QPushButton:disabled {border: 5px solid #000065; background-color: #f0f0f0; color: #000000;}";
                currButtons[49]->setStyleSheet(styleSheet);
                currButtons[61]->setStyleSheet(styleSheet);
                currShift = false;
                setNewKeyBoard();
