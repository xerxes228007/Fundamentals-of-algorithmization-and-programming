...
{
    ui->setupUi(this);
    setWindowTitle("Окно ввода данных о планете");
    setFixedSize(1400, 465);
    timer = new QTimer;
    QObject::connect(timer, &QTimer::timeout, [=](){
        if (ui->spb_age->value() == 0 || ui->dspb_weight->value() == 0 || ui->ln_dietType->text() == "" || ui->ln_species->text() == "") {
            ui->bnt_send->setStyleSheet("background-color: #E6E6FA;");
...
void Input::on_bnt_send_clicked()
{
    ...
    temp.hoursOfSleep[4] = ui->spb_6->value();
    temp.hoursOfSleep[4] = ui->spb_7->value();
