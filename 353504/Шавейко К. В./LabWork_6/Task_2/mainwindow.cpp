...
void MainWindow::printMenu(int choice)
{
    ...
    case 5:
        ...
        break;
    case 6:
        fileName = "ShaveikoLab6_1.txt";
        displayAnimalDataFromFile(fileName);
        break;
    case 7:
        fileName = "ShaveikoLab6_2.bin";
...
{
    QStringList filesToCreate = {"ShaveikoLab6.txt", "ShaveikoLab6_0.txt", "ShaveikoLab6_1.txt", "ShaveikoLab6_2.bin", "ShaveikoLab6_3.txt"};
...
void MainWindow::saveAnimalDataToFile(const QString &fileName)
{
    QFile file(fileName);
    QTextStream fileStream(&file);
    outputStream << "Вы выбрали пункт 5\nВведите данные о планете: возраст, масса, тип (один символ), существует ли (0 или 1)\n";
    ui->lineEdit->clear();
    ui->textEdit->setText(textOutput);
    ui->textEdit->moveCursor(QTextCursor::End);
    ui->lineEdit->setReadOnly(false);
    if (!file.open(QIODevice::Append | QIODevice::Text)) {
        ...
        ui->lineEdit->setText(textInput);
        ui->lineEdit->clear();
        outputStream << "Введте массу(double)";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
        ...
        ui->lineEdit->setText(textInput);
        ui->lineEdit->clear();
        outputStream << "Введте тип (один символ)";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
        ...
        ui->lineEdit->setText(textInput);
        ui->lineEdit->clear();
        outputStream << "Введите существует ли (0 или 1)";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
        ...
        if (inputStream.status() != QTextStream::Ok || (isMale != 0 && isMale != 1)) {
            ...
            textInput.remove(0, inputStream.pos());
            inputStream.seek(0);
            outputStream << (animal.isMale ? "Да" : "Нет") << '\n';
        }
        ui->lineEdit->setText(textInput);
        fileStream << animal.age << " " << animal.weight << " " << animal.dietType << " " << (animal.isMale ? "1" : "0") << "\n";
        file.close();
    }
    outputStream << "\nДанные о планете успешно записаны в файл " << fileName << "\n";
    ui->textEdit->setText(textOutput);
    ui->textEdit->moveCursor(QTextCursor::End);
}
...
void MainWindow::displayAnimalDataFromFile(const QString &fileName)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!fileStream.atEnd()) {
            ...
            outputStream << "Масса: " << animal.weight << " ; ";
            outputStream << "Тип: " << animal.dietType << " ; ";
            outputStream << "Существует ли? : " << (animal.isMale ? "Да" : "Нет") << "\n";
        }
        file.close();
    }
    ui->textEdit->setText(textOutput);
...
void MainWindow::saveAnimalDataToBinaryFile(const QString &fileName)
{
    ...
    ui->textEdit->setText(textOutput);
    ui->textEdit->moveCursor(QTextCursor::End);
    outputStream << "Вы выбрали пункт 5\nВведите данные об животном: возраст, масса, тип (один символ), существует ли (0 или 1)\n";
    ui->lineEdit->clear();
    if (!file.is_open()) {
        outputStream << "\nНе удалось открыть файл" << fileName << "для запси\n";
    } else {
        Animal animal;
        outputStream << "Введите возраст(int)";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
        ...
        ui->lineEdit->setText(textInput);
        ui->lineEdit->clear();
        outputStream << "Введите массу(double)";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
        ...
        ui->lineEdit->setText(textInput);
        ui->lineEdit->clear();
        outputStream << "Введите тип (один символ)";
        ui->textEdit->setText(textOutput);
        ui->textEdit->moveCursor(QTextCursor::End);
        ui->lineEdit->setReadOnly(false);
        while (textInput.isEmpty()){
    ...
    ui->textEdit->setText(textOutput);
    outputStream << "Данные о планете успешно записаны в бинарный файл " << fileName << "\n";
    ui->textEdit->moveCursor(QTextCursor::End);
}
