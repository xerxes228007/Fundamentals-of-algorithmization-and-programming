...
ASTSGamePlayerController::ASTSGamePlayerController()
{
...
void ASTSGamePlayerController::OnPossess(APawn* InPawn)
{
...
void ASTSGamePlayerController::PlayerTick(float DeltaTime)
{
...
void ASTSGamePlayerController::BeginPlay()
{
	Super::BeginPlay();
...
const FName ASTSGamePlayerController::MoveForwardBinding("MoveForward");
const FName ASTSGamePlayerController::MoveRightBinding("MoveRight");

void ASTSGamePlayerController::CalculateMovement(float DeltaSeconds)
{
...
void ASTSGamePlayerController::MoveToMouseCursor()
{
	if (hit.bBlockingHit)
	{
...
void ASTSGamePlayerController::MoveToTouchLocation(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
void ASTSGamePlayerController::SetNewMoveDestination(const FVector DestLocation)
{
...
void ASTSGamePlayerController::OnSetDestinationPressed()
{
	leftMouseDown = true;
	mouseStart = hit.Location;
}

void ASTSGamePlayerController::OnSetDestinationReleased()
{
	leftMouseDown = false;
	SelectUnits();
}

void ASTSGamePlayerController::AttackTarget(IDamagableInterface* target) {
	if (selectedUnits.Num() > 0) {
		for (AActor* a : selectedUnits) {
			if (a->IsA(ASTSGameCharacter::StaticClass())) {
				ASTSGameCharacter* c = Cast<ASTSGameCharacter>(a);
...
void ASTSGamePlayerController::GatherResources(IResourceInterface* res) {
	if (selectedUnits.Num() > 0) {
		for (AActor* a : selectedUnits) {
			if (a->IsA(ASTSGameCharacter::StaticClass())) {
				ASTSGameCharacter* c = Cast<ASTSGameCharacter>(a);
...
void ASTSGamePlayerController::RightClick()
{
	if (hit.bBlockingHit)
	{
		AActor* targetFound = hit.GetActor();
...
void ASTSGamePlayerController::MoveUnits(FVector loc)
{
	if (selectedUnits.Num() > 0) {
		for (AActor* a : selectedUnits) {
...
void ASTSGamePlayerController::SelectUnits()
{
	...
	if (actors.Num() > 0) {
		...
		for (AActor* a : actors) {
			if (a->IsA(ASTSGameCharacter::StaticClass())) {
				ASTSGameCharacter* character = Cast<ASTSGameCharacter>(a);
...
void ASTSGamePlayerController::ClearDelegates()
{
	if (selectedUnits.Num() > 0) {
		...
		else if (a->IsA(ASTSGameCharacter::StaticClass()))
		{
