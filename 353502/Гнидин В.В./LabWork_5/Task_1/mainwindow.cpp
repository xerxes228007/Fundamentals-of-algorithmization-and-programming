...
{
    if (!isTesting) return;
    ui->wpmLCD->setText(QString::number(1.0 * words * 60000 / timer->elapsed()));
    ui->timerLCD->setText(QString::number(timer->elapsed() / 100));
    ui->accuracyLCD->setText(QString::number(100. * rightSymbols / (allSymbols == 0 ? 1 : allSymbols)));
}
