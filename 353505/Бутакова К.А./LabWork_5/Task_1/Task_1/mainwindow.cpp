...
void MainWindow::StartTest()
{
    ...
    if (ui->GenerateTextRadioButton->isChecked()){
        ...
        if (LanguageComboBox->currentText() == "中文") ui->label->setText(ChinaMap[QString(Text[0])]);  // Преобразование QChar в QString
    ...
    if (ui->OpenTextRadioButton->isChecked() && Text != ""){
        ...
        while (!Text.isEmpty() && Text.back() == ' ') {
            Text.chop(1); // Удалить последний символ
        }
        CopyText = Text;
        CopyText.replace(" ", "_");
        ui->TextLabel->setText(CopyText);
        ...
        LanguageComboBox->setEnabled(false);
        LanguageComboBox->setPalette(Qt::gray);
        if (LanguageComboBox->currentText() == "中文") ui->label->setText(ChinaMap[QString(Text[0])]);
        timer->start(1000);
    }
}
...
{
    MapInizialisation("ButtonsNumbersBy");
    QFile file("by.txt");
...
{
    MapInizialisation("ButtonsNumbersGer");
    QFile file("deu.txt");
    file.open(QIODevice::ReadOnly);
    if (file.isOpen()){
        QString str = file.readAll();
...
{
    MapInizialisation("ButtonsNumbersFr");
    QFile file("fra.txt");
    file.open(QIODevice::ReadOnly);
    if (file.isOpen()){
        QString str = file.readAll();
...
void MainWindow::ChineseTextGenerate()
{
    ...
    Alphas.clear();
    MapInizialisation("ButtonsNumbersCh");
    QFile file("ch.txt");
    file.open(QIODevice::ReadOnly);
    if (file.isOpen()){
        QString str = file.readAll();
...
{
    MapInizialisation("ButtonsNumbersArab");
    QFile file("arab.txt");
    file.open(QIODevice::ReadOnly);
    if (file.isOpen()){
        QString str = file.readAll();
...
{
    MapInizialisation("ButtonsNumbersHebr");
    QFile file("ivrit.txt");
    file.open(QIODevice::ReadOnly);
    if (file.isOpen()){
        QString str = file.readAll();
        Alphas = str.split(" ");
...
{
    QString FileName = QFileDialog::getOpenFileName(this, "Выберите файл", "/home/LabWork_5/Task_1", "*.txt");
    if (!FileName.isEmpty()){
        QFile file(FileName);
        file.open(QIODevice::ReadOnly);
        if (file.isOpen()){
        ...
        if (LanguageComboBox->currentText() == "中文"){
            ...
            if (Text[0] == ' '){
                if (event->key() == 32){
                    ...
                    Text.remove(0, 1); // Удаляет первый символ из строки
                    CopyText = Text;
                    CopyText.replace(" ", "_");
                    ui->TextLabel->setText(CopyText);
                    ui->label->setText(ChinaMap[QString(Text[0])]);
                    NumberOfAlphas++;
                ...
                str.remove(0, 1); // Удаляет первый си
                ...
                if (event->key() == 32 && ui->label->text() == ""){
                    ...
                    CopyText.replace(" ", "_");
                    ui->TextLabel->setText(CopyText);
                    if (Text != "") ui->label->setText(ChinaMap[QString(Text[0])]);
                    NumberOfAlphas++;
                    ui->Alphaslabel->setText("Количество знаков " + QString::number(NumberOfAlphas));
                    ui->ProzentLabel->setText("Точность " + QString::number(100 * NumberOfAlphas/(double(NumberOfAlphas + NumberOfErrors))) + "%");
                    if (Text == "") EndTest();
