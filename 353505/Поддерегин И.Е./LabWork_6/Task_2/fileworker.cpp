...
void FileWorker::SaveStructToFile(const QString &fileName)
{
    ...
    else
    {
        ...
        for (int i = 0; i < a.MAX_POST_SIZE; ++i)
        {
            text += QString(a.post[i]) + "\n";
        }
        fileStream << text;
        file.close();
    }
...
void FileWorker::LoadStructsFromFile(const QString &fileName)
{
    ...
    else
    {
        while (!fileStream.atEnd())
        {
            ...
            temp.size = data[2][0].toLatin1();
            temp.has = data[3] == "true" ? true : false;
            for (int i = 0; i < temp.MAX_NAME_SIZE - 1; ++i)
            {
                temp.name[i] = data[4][i].toLatin1();
                temp.name[i + 1] = '\0';
            }
            for (int i = 0; i < temp.MAX_POST_SIZE; ++i)
            {
                temp.post[i] = data[i + 5].toInt();
            }

            outputStream << temp.year << "|" << temp.cost << "|" << QString(temp.size) << "|" << (temp.has ? "true" : "false") << "|" << QString(temp.name) << "|";
            for (int i = 0; i < temp.MAX_POST_SIZE; ++i)
            {
                outputStream << temp.post[i] << (i == temp.MAX_POST_SIZE - 1 ? "\n" : "|");
            }
        }
        file.close();
    }
