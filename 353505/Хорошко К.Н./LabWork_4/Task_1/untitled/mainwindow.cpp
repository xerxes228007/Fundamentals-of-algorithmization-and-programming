    ...
    if (ok){
        ...
        if (in){
            if (k == n){
               ...
               QElapsedTimer timer;
                timer.start();
                sort::quick_sort(arr, 0, n-1, max, rect, 10, 100, this, n);
                QString str = "quick " + QString::number((double)timer.elapsed()/1000);
                timer.invalidate();
                ui->label_4->setText(str);
                timer.start();
                sort::heapSort(arr1, n, max, rect1, 420, 100, this, n);
                str = "heap " + QString::number((double)timer.elapsed()/1000);
                timer.invalidate();
                ui->label_5->setText(str);
                timer.start();
                sort::mergeSort(arr2, 0, n-1, max, rect2, 830, 100, this, n);
                str = "merge " + QString::number((double)timer.elapsed()/1000);
                timer.invalidate();
