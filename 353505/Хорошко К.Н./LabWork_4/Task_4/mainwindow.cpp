...
{
    if (data.empty()) {
        QMessageBox::warning(this, "Empty Data", "No data available to display.");
        return;
    }
    ...
    for (int i = 0; i < colCount1; ++i) {
        headers << QString("Vector 1, Col %1").arg(i + 1);
    }
    for (int i = 0; i < colCount2; ++i) {
        headers << QString("Vector 2, Col %1").arg(i + 1);
    }
...
void MainWindow::initializeData()
{
    ...
    Vector<pair<int, double>> pairVector1 = {{1, 1.1}, {2, 2.2}, {3, 3.3}};
    Vector<pair<int, double>> pairVector2 = {{4, 4.4}, {5, 5.5}, {6, 6.6}};
    Vector<pair<int, double>> pairVector3 = {{7, 7.7}, {8, 8.8}, {9, 9.9}};
    data.push_back({vector1, pairVector1});
    data.push_back({vector2, pairVector2});
    data.push_back({vector3, pairVector3});
}
