...
void BattleShipView::changeEvent(QEvent *event)
{
    if (event->type() == QEvent::LanguageChange) {
        ...
        mainMenu.m_pButtonSinglePlayer->setText(tr("Одиночная игра"));
        mainMenu.m_pButtonMultiPlayer->setText(tr("онлайн игра"));
        mainMenu.m_pButtonStatistics->setText(tr("Статистика"));
        mainMenu.m_pButtonExit->setText(tr("Выход"));

        // singleplayer menu update
        singlePlayerMenu.m_pSinglePlayerTitle->setText(tr("Одиночная игра"));
        singlePlayerMenu.m_pButtonStartGame->setText(tr("Начать игру"));
        singlePlayerMenu.m_pButtonSelectDifficulty->setPrefix(tr("Сложность"));
        singlePlayerMenu.m_pButtonSelectDifficulty->updateTranslate(tr("Просто"), Bot::Easy);
        singlePlayerMenu.m_pButtonSelectDifficulty->updateTranslate(tr("Сложно"), Bot::Medium);
        singlePlayerMenu.m_pButtonClearMap->setText(tr("Очистить карту"));
        singlePlayerMenu.m_pButtonGenerateNewMap->setText(tr("Сгенерировать новую карту"));
        singlePlayerMenu.m_pGameMapEditor->setLabelText(tr("ABCDEFGHIJ"));
        singlePlayerMenu.m_pButtonSinglePlayerBack->setText(tr("Назад"));

        // game menu update
        gameMenu.m_pHumanMap->setLabelText(tr("ABCDEFGHIJ"));
        gameMenu.m_pBotMap->setLabelText(tr("ABCDEFGHIJ"));
        gameMenu.m_pButtonGameMenuBack->setText(tr("Назад"));
    } else {
