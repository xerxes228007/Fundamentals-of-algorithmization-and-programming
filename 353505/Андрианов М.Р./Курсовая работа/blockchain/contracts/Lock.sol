...
contract Lock {
    ...
    uint public totalVotes = 0; // Общее количество отданных голосов
    ...
    event WinnerDeclared(uint candidateID, string name, string description, uint voteCount, uint percent);
    ...
    function addCandidate(string memory name, string memory description) public onlyAdmin {
        require(!alreadyExists[keccak256(abi.encodePacked(name, description))], "Candidate with the same name and description already exists");
        require(!votingStarted, "Voting has already started");
        candidatesCount++;
        candidates[candidatesCount] = Candidate(candidatesCount, name, description, 0);
        alreadyExists[keccak256(abi.encodePacked(name, description))] = true; // Отметить имя и партию как добавленные
    ...
    function vote(uint candidateID) public {
        ...
        voters.push(msg.sender); // Сохраняем адрес голосующего
    ...
    function resetVoting() private {
        ...
        for (uint i = 1; i <= candidatesCount; i++) {
            ...
            bytes32 namedescriptionKey = keccak256(abi.encodePacked(candidate.name, candidate.description));
    ...
    function declareWinner() private {
        ...
        uint[] memory winners = new uint[](candidatesCount); // Инициализируем массив с максимально возможным размером
        uint winnersCount = 0;
        ...
        for (uint i = 0; i < winnersCount; i++) {
            ...
            uint winnerPercent = (winningVoteCount * 100) / totalVotes;
