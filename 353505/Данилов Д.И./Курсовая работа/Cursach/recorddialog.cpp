...
void RecordLabel::setGeometry(int lavelIndex, int recordIndex){
    int x = INIT_POSITION_X + DISTANCE_X*lavelIndex;
...
void RecordLabel::updateRecordText(){
    if(isHaveRecord) {
        setText(QString("%1s (%2)").arg(time).arg(name));
        setStyleSheet(QString("font-size: %1px").arg(13));
    }
}
