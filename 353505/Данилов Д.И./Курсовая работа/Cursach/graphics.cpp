...
void GraphicsWidget::paintBackgroundInner(QPainter &painter){
    ...
    int heidth = pGame->getTableCols()*GameBlock::SIZE +INNER_BORDER +INNER_BORDER;

    painter.fillRect(MARGIN - INNER_BORDER, MARGIN - INNER_BORDER, width, heidth, QBrush(colors.gray));
}
...
void GraphicsWidget::paintBackgroundOuter(QPainter &painter){
    ...
    int heidth = pGame->getTableCols()*GameBlock::SIZE +OUTER_BORDER +OUTER_BORDER;

    painter.fillRect(MARGIN - OUTER_BORDER, MARGIN - OUTER_BORDER, width, heidth, QBrush(colors.black));
}
